...
make_init <- function(.data, .args) {
  ...
  function(){
    list(mean_metabolite = rnorm(1, 10, 0.25),
         ...
         dmean_metabolite = rnorm(1, 10, 0.25),
         dmean_dilution  = rnorm(1, 1, 0.25),
         dmean_injection  = rnorm(1, 1, 0.25),
         ...
         dmean_log_sigma = rnorm(1, -1, 0.25),
         ...
         z_chrom1 = rep(0,.data$number_chroms),
         z_chrom2 = rep(0,.data$number_chroms),
         sigma_shared = exp(rnorm(2, log(0.5), 0.25)),
         r_ms = matrix(c(r_metabolite_init, rep(-1,.data$number_metabolites)),ncol = 2),
         z_di = matrix(c(rep(0,.data$number_metabolites), rep(0,.data$number_metabolites)),ncol = 2),
         r_injection = rep(1,.data$number_metabolites),
         z_dilution_injection_interaction = rep(0,.data$number_metabolites),
